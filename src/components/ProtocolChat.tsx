  ...
  const sendQuestion = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_LARAVEL_API_URL}/api/questions`,
        {
          method: 'POST',
          headers: {
      ...
      <form onSubmit={sendQuestion} className="p-4 border-t">
        ...
        {connectionStatus !== 'connected' && (
          <div className="text-xs text-gray-500 mt-1">
            Status: {connectionStatus} - Input disabled until connected
          </div>
        )}
      </form>
    </div>
...
function MessageItem({ message }: MessageItemProps) {
  const getMessageClasses = (): string => {
    ...
    switch (message.type) {
      case 'question':
        return `${baseClasses} ${
          message.operator ? 'bg-red-50 border border-red-200' : 'bg-blue-50'
        }`;
      case 'reply':
  ...
  return (
    <div className={getMessageClasses()}>
      ...
      <div className="message-content text-gray-800 whitespace-pre-wrap">
        {message.content}
      </div>
